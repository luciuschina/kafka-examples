...
public class StreamingAssert {
    ...
    List<Record> readRecords() throws Exception {
        ...
        job.set(HiveConf.ConfVars.HIVE_TRANSACTIONAL_TABLE_SCAN.varname, "true");
        job.set(ValidTxnList.VALID_TXNS_KEY, txns.toString());
        InputSplit[] splits = inputFormat.getSplits(job, 1);
        assertEquals(1, splits.length);

        final AcidInputFormat.AcidRecordReader<NullWritable, OrcStruct> recordReader = (AcidInputFormat.AcidRecordReader<NullWritable, OrcStruct>) inputFormat
